...
class SkyElevenAgent:
    ...
    async def run_cycle(self):
        ...
        # 3. AUDITAR y CALIFICAR con IA
        calificados = []
        for prospect in nuevos:
            # Senior Rule: Si tiene web, la auditamos para encontrar 'ganchos'
            if prospect.get("website"):
                try:
                    log.info(f"🔍 Auditoría técnica para: {prospect['nombre_negocio']}")
                    audit_res = await self.auditor.auditar(prospect["website"])
                    prospect["audit_data"] = audit_res
                    prospect["descripcion"] += f"\n[Auditoría]: {audit_res.get('gancho_venta', '')}"
                    
                    # SI NO TENÍA TELÉFONO/EMAIL Y LA AUDITORÍA LOS ENCONTRÓ, LOS AGREGAMOS
                    if not prospect.get("telefono") and audit_res.get("telefono"):
                        prospect["telefono"] = audit_res["telefono"]
                        log.info(f"   📱 Teléfono encontrado en web: {prospect['telefono']}")
                    if not prospect.get("email") and audit_res.get("email"):
                        prospect["email"] = audit_res["email"]
                        log.info(f"   📧 Email encontrado en web: {prospect['email']}")
                except Exception as e:
                    log.error(f"Error en auditoría para {prospect['nombre_negocio']}: {e}")

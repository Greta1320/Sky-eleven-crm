...
class ScraperAdapter:
    ...
    async def _scrape_instagram(self) -> List[Dict]:
        ...
        try:
            ...
            cuentas_comp_str = os.getenv("INSTAGRAM_CUENTAS_COMPETENCIA", "")
            
            cuentas_directorio = [c.strip() for c in cuentas_dir_str.split(",") if c.strip()]
            cuentas_competencia = [c.strip() for c in cuentas_comp_str.split(",") if c.strip()]
            ...
            raw = await asyncio.to_thread(
                ...
                cuentas_competencia=cuentas_competencia or None
            )
            
            # Si hay posts de competencia configurados, también corremos el sniper de comentarios
            posts_sniper = os.getenv("INSTAGRAM_POSTS_SNIPER", "").split(",")
            if posts_sniper and posts_sniper[0]:
                log.info(f"🎯 Ejecutando sniper en {len(posts_sniper)} posts...")
                leads_comentarios = await asyncio.to_thread(
                    scraper.buscar_en_comentarios,
                    post_urls=[p.strip() for p in posts_sniper if p.strip()]
                )
                raw.extend(leads_comentarios)
